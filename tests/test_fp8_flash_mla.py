...
def test_flash_mla(b, s_q, mean_sk, h_q, h_kv, d, dv, causal, varlen, torch_dtype):
    ...
    seed = 42                     # 任选一个整型种子
    torch.manual_seed(seed)       # 固定 CPU 生成器
    torch.cuda.manual_seed_all(seed)  # 如需在 GPU 上保持一致，再加这一行

    q = torch.randn(b, s_q, h_q, d)
    #q = torch.full((b, s_q, h_q, d), 0.02)
    block_size = 64
    block_table = torch.arange(
        b * max_seqlen_pad // block_size, dtype=torch.int32
    ).view(b, max_seqlen_pad // block_size)
    blocked_k = torch.randn(block_table.numel(), block_size, h_kv, d)
    #blocked_k = torch.full((block_table.numel(), block_size, h_kv, d), 1.0)
    for i in range(b):
    ...
    def prepare_fp8_input():
        ...
        if use_fp8:
            ...
            # descale_q = torch.ones((1), dtype=torch.float32, device="cpu") // cpu scalar faster ,but need change sglang api used
            # descale_k = torch.ones((1), dtype=torch.float32, device="cpu") // cpu scalar faster ,but need change sglang api used
    ...
    descale_q_gpu = descale_q.to(q.device)
    descale_k_gpu = descale_k.to(q.device)
    ...
    if use_fp8:
        ...
        #q = torch.ones(q_fp8)
        #blocked_k = torch.ones(blocked_k_fp8)
        #blocked_v = torch.ones(blocked_v_fp8)
    ...
    def ref_mla():
        q_ = (q.to(torch.float) * descale_q_gpu).to(init_dtype) if use_fp8 else q
        blocked_k_ = (blocked_k.to(torch.float) * descale_k_gpu).to(init_dtype) if use_fp8 else blocked_k
        blocked_v_ = (blocked_v.to(torch.float) * descale_k_gpu).to(init_dtype) if use_fp8 else blocked_v
