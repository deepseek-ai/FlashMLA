...
template<
    ...
    typename Tensor2
>
__forceinline__ __device__ void scale_softmax_packed(
    Tensor0 &rP,
    Tensor1 &rS,
    Tensor2 &rO,
    float scale_softmax_log2,
    ...
    for (int local_row_idx = 0; local_row_idx < 2; ++local_row_idx) {
        ...
        for (int i = 0; i < size(cur_rP); ++i) {
            if constexpr (MASK_TAIL) {
                const int token_in_block =
                    (i&1) + (i/2)*8 + (idx_in_warpgroup%4)*2;
                if (token_in_block >= num_valid_tokens) {
                    cur_rP(i) = -INFINITY;
                }
            }
            cur_max = max(cur_max, cur_rP(i));
        ...
        for (int i = 0; i < size(cur_rP); ++i) {
            cur_rP(i) =
                exp2f(cur_rP(i)*scale_softmax_log2 - rM[local_row_idx]);
            cur_rS(i) = (bf16)cur_rP(i);
            cur_sum += cur_rP(i);
        }
    ...
    if (idx_in_warpgroup%4 == 0) {
        *(float2*)(sScale + 2*(idx_in_warpgroup/4)) =
            *(float2*)(scale_for_olds);
    }
}

template<ModelType MODEL_TYPE, int NUM_HEADS, bool PACKED_KV>
template<typename TMAParams>
__device__ void KernelTemplate<MODEL_TYPE, NUM_HEADS, PACKED_KV>::devfunc(
    ...
    auto get_cur_req_info = [&](int batch_idx) -> MainloopArgs {
        ...
        if constexpr (MODEL_TYPE == ModelType::V32) {
            if constexpr (PACKED_KV) {
                args.topk_length = params.topk_length
                    ? __ldg(params.topk_length + batch_idx)
                    : params.topk;
                total_topk_padded = max(
                    ku::ceil(args.topk_length, (int)TOPK_BLOCK_SIZE),
                    (int)TOPK_BLOCK_SIZE
                );
            } else {
    ...
    if (warpgroup_idx == 0) {
        ...
        for (int batch_idx = sched_meta.begin_req_idx; batch_idx <= sched_meta.end_req_idx; ++batch_idx) {
            ...
            for (int block_idx = args.start_block_idx; block_idx < args.end_block_idx; block_idx++) {
                ...
                if constexpr (PACKED_KV) {
                    const int num_valid_tokens = min(
                        max(
                            args.topk_length - block_idx * TOPK_BLOCK_SIZE,
                            0
                        ),
                        (int)TOPK_BLOCK_SIZE
                    );
                    if (num_valid_tokens < TOPK_BLOCK_SIZE) {
                        scale_softmax_packed<true>(
                            rP, rS, rO, params.sm_scale_div_log2,
                            sScale, rM, rL, idx_in_warpgroup,
                            num_valid_tokens
                        );
                    } else {
                        scale_softmax_packed<false>(
                            rP, rS, rO, params.sm_scale_div_log2,
                            sScale, rM, rL, idx_in_warpgroup,
                            TOPK_BLOCK_SIZE
                        );
                    ...
                    scale_softmax(
                        rP, rS, rO, params.sm_scale_div_log2,
                        sScale, rM, rL, plan.is_kv_valid[buf_idx],
                        block_idx, idx_in_warpgroup
                    );
                }
        ...
        for (int batch_idx = sched_meta.begin_req_idx; batch_idx <= sched_meta.end_req_idx; ++batch_idx) {
            MainloopArgs args = get_cur_req_info(batch_idx);
            int* gIndices = PACKED_KV
                ? nullptr
                : params.indices
                    + batch_idx*params.stride_indices_b
                    + s_q_idx*params.stride_indices_s_q; // (topk) : (1)
            int* gExtraIndices = PACKED_KV
                ? nullptr
            ...
            auto process_one_block = [&](int block_idx, auto is_extra_block_t, auto is_first_extra_block_t) {
                ...
                [[maybe_unused]] static constexpr bool IS_FIRST_EXTRA_BLOCK =
                ...
                if constexpr (!IS_EXTRA_BLOCK) {
                    indices_base = PACKED_KV
                        ? nullptr
                        : gIndices + (block_idx)*TOPK_BLOCK_SIZE;
                ...
                for (int round = 0; round < NUM_TOKENS_PER_THREAD; ++round) {
                    ...
                    if constexpr (PACKED_KV) {
                        block_index =
                            (batch_idx*params.s_q + s_q_idx)
                                * (params.topk/TOPK_BLOCK_SIZE)
                            + block_idx;
                        rel_idx_in_block =
                            idx_in_cluster*(TOPK_BLOCK_SIZE/2) + my_token_idx;
                    } else {
                        block_index = token_index == -1 ? 0 : (int)((uint32_t)token_index/(uint32_t)page_block_size);   // Use uint32_t division and mod to improve performance
                        rel_idx_in_block = (uint32_t)token_index % (uint32_t)page_block_size;   // NOTE When token_index is -1 (UINT_MAX), UINT_MAX%page_block_size < page_block_size, so there will be no illegal-memory-access error
                    }
                    ...
                    if constexpr (MODEL_TYPE == ModelType::V32) {
                        ...
                        gK_base = PACKED_KV
                            ? k_ptr + block_index*k_block_stride
                            : k_ptr + block_index*k_block_stride
                                + rel_idx_in_block*k_row_stride;
                        float scales_float[NUM_SCALES];
                        float* gK_scales = PACKED_KV
                            ? (float*)(
                                gK_base
                                + TOPK_BLOCK_SIZE*HEAD_DIM_NOPE
                                + rel_idx_in_block
                    ...
                    for (int dim_idx = 0; dim_idx < HEAD_DIM_NOPE/64; dim_idx += 1) {
                        fp8* gK_nope = PACKED_KV
                            ? gK_base
                                + (
                                    (dim_idx*4 + lane_idx/8)
                                        * TOPK_BLOCK_SIZE
                                    + rel_idx_in_block
                                )*16
                            : gK_base + (lane_idx/8)*16 + dim_idx*64;
                        fp8x16 cur_fp8x16 = load_128b_from_gmem<fp8x16, L1CacheHint::EVICT_LAST, L2PrefetchHint::B256>(gK_nope);   // We use EVICT_LAST here since gK_base may not be aligned to 32B (for V3.2) and the performance is the best among all cache hints (for MODEL1)
                    ...
                    for (int dim_idx = 0; dim_idx < HEAD_DIM_ROPE/32; dim_idx += 1) {
                        bf16* gK_rope_chunk = PACKED_KV
                            ? gK_rope
                                + dim_idx*4*TOPK_BLOCK_SIZE*8
                            : gK_rope + dim_idx*32;
                        bf16x8 cur_bf16x8 = load_128b_from_gmem<bf16x8, L1CacheHint::EVICT_LAST, L2PrefetchHint::B128>(gK_rope_chunk);
                        if constexpr (MODEL_TYPE == ModelType::V32) {
                ...
                if (idx_in_warpgroup < 32) {
                    ...
                    if constexpr (!PACKED_KV) {
                        int2 indices =
                            __ldg((int2*)(indices_base + lane_idx*2));
                        *(char2*)(
                            &plan.is_kv_valid[buf_idx][lane_idx*2]
                        ) = {
...
template<ModelType MODEL_TYPE, int NUM_HEADS, bool PACKED_KV>
void KernelTemplate<MODEL_TYPE, NUM_HEADS, PACKED_KV>::run(
    const SparseAttnDecodeParams &params
) {
    ...
    if constexpr (PACKED_KV) {
        ...
        KU_ASSERT(params.page_block_size == TOPK_BLOCK_SIZE);
        KU_ASSERT(
            params.num_blocks
            == params.b * params.s_q
                * (params.topk / TOPK_BLOCK_SIZE)
    ...
    auto mla_kernel = &flash_fwd_splitkv_mla_fp8_sparse_kernel<
        KernelTemplate<MODEL_TYPE, NUM_HEADS, PACKED_KV>,
        decltype(tma_params)
    >;
