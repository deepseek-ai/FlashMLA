...
#include "sm100/decode/sparse_fp8_smallbatch/splitkv_mla.h"
...
static constexpr int SMALLBATCH_THRESHOLD = 8;
...
protected:
    void run_(const SparseAttnDecodeParams &params, const std::vector<FeatureT> &required_features) override {
        ...
        if (
            ...
            params.b < SMALLBATCH_THRESHOLD
        ) {
            TORCH_CHECK(params.attn_sink == nullptr, "smallbatch sparse-fp8 kernel does not support attn_sink");
            TORCH_CHECK(params.topk_length == nullptr, "smallbatch sparse-fp8 kernel does not support topk_length");
            TORCH_CHECK(params.extra_kv == nullptr, "smallbatch sparse-fp8 kernel does not support extra_kv");
            TORCH_CHECK(params.extra_indices == nullptr, "smallbatch sparse-fp8 kernel does not support extra_indices");
            TORCH_CHECK(params.extra_topk_length == nullptr, "smallbatch sparse-fp8 kernel does not support extra_topk_length");

            auto compat_params = nv_smallbatch::sm100::make_decoding_params_compat(params);
            nv_smallbatch::sm100::run_flash_splitkv_mla_fp8_sparse_kernel(compat_params);
            return;
        }
