...
__forceinline__ __device__ void warpgroup_cooperative_qkt_gemm(
    ...
    #define QKT_GEMM_ONE_TILE(TILE_IDX) \
        qkt_gemm_one_tile_sQ<T>(tiled_mma_sQ, thr_mma_sQ_tiled(_, _, _, Int<TILE_IDX>{}), thr_mma_sKV_tiled(_, _, _, Int<TILE_IDX>{}), rP, sKV.data().get().get(), sVt_ptr, barriers + TILE_IDX, cur_phase, idx_in_warpgroup, TILE_IDX); 
...
template<
    ...
    typename Engine0, typename Layout0
>
__forceinline__ __device__ void load_sP_to_rPb(
    uint16_t* sP_ptr,
    Tensor<Engine0, Layout0> &rPb,
    int idx_in_warpgroup
) {
...
__forceinline__ __device__ void wg0_subroutine(
    ...
    //Tensor tOrP_acc = make_tensor(rP0.data(), convert_layout_acc_Aregs<typename T::TiledMMA_PV_LocalP>(rP0.layout()));
...
__forceinline__ __device__ void wg1_subroutine(
    ...
    //Tensor tOrP_acc = make_tensor(rP1.data(), convert_layout_acc_Aregs<typename T::TiledMMA_PV_LocalP>(rP1.layout()));
...
flash_fwd_splitkv_mla_kernel(__grid_constant__ const Flash_fwd_mla_params params, __grid_constant__ const TmaParams tma_params) {
    ...
    auto sV0 = [&]{
        if constexpr (T::IsFp8) {
            ...
            return make_tensor(make_smem_ptr(reinterpret_cast<typename T::InputT*>(plan.smem_sK0)),
                            typename T::SmemLayoutV_Trans{});
        }
    }();
    auto sV1 = [&]{
        if constexpr (T::IsFp8) {
            ...
            return make_tensor(make_smem_ptr(reinterpret_cast<typename T::InputT*>(plan.smem_sK1)),
                            typename T::SmemLayoutV_Trans{});
        }
    }();
    ...
    for (int batch_idx = begin_idx; batch_idx <= end_idx; ++batch_idx) {
        ...
        // Clear buffers
        cute::clear(rO);  // better than fill(rO, 0.)
...
void run_flash_splitkv_mla_kernel(Flash_fwd_mla_params &params, cudaStream_t stream) {
    using TYPE = Traits<InputT, OutputT>;
